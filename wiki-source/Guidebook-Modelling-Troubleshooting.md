...
|---|---|---|
| Registration returns false, no item | `ObjImport: No .obj path` | `model` must end in `.obj` (case-insensitive) |
| Registration returns false | `ObjImport: File not found` | Filename typo or wrong subfolder; `model` is relative to the (sub)folder you passed |
| Registration returns false | `ObjImport: File too large (… limit 67108864)` | Decimate/compress below 64 MB, re-export |
| Registration returns false | `Path escapes the pack folder` | No `..`, `/`, or absolute paths in `model`/`subfolder` — keep files inside the mod folder |
| Registration returns false | `Import failed` / `Import returned null` / `Import has no vertices` | Broken OBJ: re-export with normals + UVs, triangulated, no zero-area faces (see [[Guidebook Modelling OBJ Blender]]) |
